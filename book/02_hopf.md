...
### Lab 2.A — Hopf image of a unit quaternion
...
**Note.** `sample_fiber` sweeps \(\xi_2\) at fixed \((\eta,\xi_1)\) and returns stereographic curves used throughout the portal. For a textbook check that a true classical fiber lies over one \(\mathbb{CP}^1\) point, apply a common complex phase to a fixed \((z_1,z_2)\) (Exercise 2.C) rather than expecting bit-identical constancy of every real-component convention.

### Lab 2.C — Visualize in the portal style

Open the Gradio **Hopf Visualizer** tab, load **Classic Hopf**, and identify:

1. which panel is the **base \(S^2\)** markers,  
2. which panels are **linked-fiber projections** (xy / xz),  
3. which panel (if present) is a **phase** view.

Compare with Figures 2.2–2.3.

### Lab 2.D — Phase sweep on one fiber
...
Optionally animate `xi2` in a notebook or use portal phase controls. Auxiliary Figure A2.1 is the static version of this lab.

### Lab 2.E — Family of linked fibers
